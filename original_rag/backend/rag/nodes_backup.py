...
class RAGNodes:
    ...
    async def route_query(self, state: RAGState) -> dict:
        """Router Agent - Classifies query complexity to skip unnecessary steps."""
        logger.info(f"Routing query: {state['question'][:50]}...")
        
        chat_history = ""
        if self.memory:
            history = await self.memory.get_history(state["session_id"], limit=3)
        ...
        if "SIMPLE" in classification:
            ...
            query_complexity = "conversational"
            skip_rewrite = False
        else:
            query_complexity = "complex"
            skip_rewrite = False
        
        logger.info(f"Query classified as: {query_complexity}, skip_rewrite: {skip_rewrite}")
    ...
    async def grade_documents(self, state: RAGState) -> dict:
        """
        Grade documents using cross-encoder reranker.
        
        Takes initial_k (50) documents, reranks, returns final_k (5) to LLM.
        """
        ...
        if reranker:
            try:
                ...
                doc_texts = [d["content"] for d in state["retrieved_documents"]]
                ...
                for orig_idx, score in reranked:
                    doc = state["retrieved_documents"][orig_idx]
                ...
                return {
                    ...
                    "sources": sources,
                    "processing_steps": ["grade_reranker"],
                }
        ...
        return {
            "relevant_documents": relevant_documents,
            "sources": sources,
            "processing_steps": ["grade_threshold"],
        }
